...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
      ...
      var omniBridgeBalance = await token.balanceOf(omniBridge.address)
      expect(omniBridgeBalance).to.be.equal(aliceDepositAmount)
      ...
      expect(poolBalance).to.be.equal(utils.parseEther('0.1'))
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const aliceAddress = aliceKeypair.address() // contains only public key
    ...
    expect(aliceChangeUtxo.amount).to.be.equal(utils.parseEther('0.07'))
